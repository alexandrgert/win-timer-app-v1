#!/usr/bin/env python3
"""Render right column (timer + focus) to PNG for UI preview."""
...
def main() -> int:
    ...
    task = controller.create_task("ЮНИМ оценка", description="Пример описания задачи", start_now=True)
    ...
    window.refresh_ui()
    window.resize(320, 920)
    window.show()
    app.processEvents()

    output = ROOT / "docs/assets/focus-panel-preview.png"
    output.parent.mkdir(parents=True, exist_ok=True)
    if not window.right_column.grab().save(str(output)):
